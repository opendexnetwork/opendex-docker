...
OPTIONS:
    ...
    --disable-update                            Skip update checks and enter opendexd-ctl shell directly
...
Opendexd options:
    --opendexd.expose-ports <port>[,<port>]          Expose opendexd service ports to your host machine
    --opendexd.preserve-config                       Preserve opendexd opendexd.conf file during updates
...
function get_cloud_image() {
    ...
    echo "$RESP" | sed -E 's/.*com.opendexnetwork.image.created":"([^"]*)".*/\1/g'
}
...
function get_local_image() {
    ...
    docker image inspect -f '{{index .Config.Labels "com.opendexnetwork.image.created"}}' "$1"
}
...
function ensure_utils_image() {
    ...
    if [[ $DEV == "true" ]]; then
        UTILS_IMG=$(get_branch_image "opendexnetwork/utils:latest")
        return
    fi

    if [[ $NETWORK == "mainnet" && $BRANCH == "master" ]]; then
        I_IMG="opendexnetwork/utils:$UTILS_TAG"
    else
        I_IMG="opendexnetwork/utils:latest"
    fi
