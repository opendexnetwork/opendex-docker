...
func (t *Service) GetInfo(ctx context.Context) (*Info, error) {
	output, err := t.Exec(ctx, "opendexd-cli", "getinfo", "-j")
	if err != nil {
		return nil, err
	}
...
func (t *Service) GetStatus(ctx context.Context) (string, error) {
	...
	if err != nil {
		if err, ok := err.(service.ErrExec); ok {
			if strings.Contains(err.Output, "ond is locked") {
				nodekey := filepath.Join(t.DataDir, "nodekey.dat")
				if _, err := os.Stat(nodekey); os.IsNotExist(err) {
					return "Wallet missing. Create with opendexd-cli create/restore.", nil
				}
				return "Wallet locked. Unlock with opendexd-cli unlock.", nil
			} else if strings.Contains(err.Output, "tls cert could not be found at /root/.opendexd/tls.cert") {
				return "Starting...", nil
			} else if strings.Contains(err.Output, "ond is starting") {
				return "Starting...", nil
...
func (t *Service) Apply(cfg interface{}) error {
	...
	t.Volumes = append(t.Volumes, fmt.Sprintf("%s:/root/.opendexd", t.DataDir))
