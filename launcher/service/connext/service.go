...
var (
	...
	// This is a placeholder mnemonic that is required for startup, but is not being used since it will be overwritten by opendexd.
...
func (t *Service) Apply(cfg interface{}) error {
	...
	if t.UseVector() {
		...
		switch network {
		case types.Simnet:
			t.Environment["CONNEXT_ETH_PROVIDER_URL"] = "http://connext.simnet.opendexnetwork.com:8545"
			t.Environment["CONNEXT_NODE_URL"] = "https://connext.simnet.opendexnetwork.com"
		case types.Testnet:
			t.Environment["CONNEXT_ETH_PROVIDER_URL"] = ethProvider
			t.Environment["CONNEXT_NODE_URL"] = "https://connext.testnet.opendexnetwork.com"
		case types.Mainnet:
			t.Environment["CONNEXT_ETH_PROVIDER_URL"] = ethProvider
