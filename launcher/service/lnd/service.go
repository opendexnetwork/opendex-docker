...
func (t *Service) GetLndPid(ctx context.Context) uint16 {
	output, err := t.Exec(ctx, "pgrep", "lnd")
	if err != nil {
		return 0
	}

	pid, err := strconv.ParseInt(output, 10, 64)
	if err != nil {
		return 0
	}
	return uint16(pid)
	...
	if network == types.Simnet {
		...
		case Bitcoin:
			t.Command = append(t.Command,
				...
				"--neutrino.connect=btcd.simnet.opendexnetwork.com:38555",
		...
		case Litecoin:
			t.Command = append(t.Command,
				...
				"--routing.assumechanvalid",
				"--neutrino.connect=btcd.simnet.opendexnetwork.com:39555",
				"--chan-enable-timeout=0m10s",
...
func (t *Service) Rescue(ctx context.Context) bool {
	...
	if err := t.RemoveData(ctx); err != nil {
		return false
	}
