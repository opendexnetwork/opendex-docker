...
def get_github_job_url(run_id: str, job_name: str) -> Optional[str]:
    url = "https://api.github.com/repos/opendexnetwork/opendex-docker/actions/runs/{}/jobs".format(run_id)
    resp = urlopen(url)
    j = json.load(resp)
    for job in j["jobs"]:
        if job["name"] == job_name:
            return "https://github.com/opendexnetwork/opendex-docker/runs/{}?check_suite_focus=true".format(job["id"])
    return None
